...
import torchvision
import numpy as np
from skimage.morphology import skeletonize, medial_axis, thin, remove_small_objects, binary_erosion, binary_dilation
...
def get_river_banks(river):
    """
    Return only the true river banks (both sides) while ignoring island edges.
    Strategy:
      - boundary = river XOR erode(river)  (morphological gradient)
      - label(~river), find background components touching the image border
      - islands = background components that DO NOT touch border
      - banks = boundary AND not-adjacent-to-islands
    """
    river = river.astype(bool)
    boundary = river ^ binary_erosion(river)
    ...
    islands = (lbl > 0) & (~np.isin(lbl, border_labels))
    banks = boundary & (~binary_dilation(islands))

    return np.column_stack(np.where(banks))


def compute_river_width(river, centerline, boundary_points, spatial_res):
    ...
    centerline_points = np.column_stack(np.where(centerline)) 
    ...
    for r, c in sampled_points:
        width = 2 * distance_transform[r, c] * spatial_res 
        widths.append(width)
...
input_image_path = "C:\\Users\\fa578s\\Desktop\\preprocessed\\GLH\\train\\img\\75.jpg"
ground_truth_path = "c:\\Users\\fa578s\\Desktop\\preprocessed\\GLH\\test\\label\\72.png"
...
boundary_points = get_river_banks(river)
point, widths, closest_boundary_points = compute_river_width(river, centerline, boundary_points, 0.3)


plt.imshow(river, cmap='gray')
plt.contour(centerline, colors='r')
plt.scatter(boundary_points[:, 1], boundary_points[:, 0], c="blue", s=2, label="Boundary Points")
plt.scatter(point[:, 1], point[:, 0], c=widths, cmap="jet", s=40, edgecolors="black")  # Color by width
# Draw width lines
